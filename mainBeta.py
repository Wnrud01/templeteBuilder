...
# --- [수정된 부분] generation_prompt는 오직 '창작'에만 집중 ---
generation_prompt = ChatPromptTemplate.from_template(
    """당신은 창의적이고 실수를 통해 배우는 알림톡 템플릿 전문가입니다.
    당신의 유일한 임무는 아래 지시사항에 따라 **완벽한 템플릿 텍스트 하나만**을 생성하는 것입니다.
    템플릿 외에 다른 설명은 절대로 덧붙이지 마세요.
...
# --- [수정된 부분] validation_prompt가 '판단'과 '설명'의 모든 책임을 짐 ---
validation_prompt = ChatPromptTemplate.from_template(
    """당신은 과거 판례까지 참고하는 매우 꼼꼼한 최종 심사관입니다.
    ...
    3. **[위반 사항이 없을 경우]**
        - "accept" 라고만 답변하세요.
    4. **[위반 사항이 있을 경우]**
        - 어떤 규정 또는 과거 사례를 근거로 위반인지 구체적으로 설명해주세요.
        - 만약 광고성 내용(할인, 쿠폰, 이벤트 등)이 문제라면, "광고성 메시지로 판단됨." 이라고 명시하고 '친구톡' 사용을 권장하세요.
    """
)
...
def route_request(state: GraphState):
    ...
    if "draft" in request_type.lower():
        ...
        state['correction_attempts'] = 0 # 검증 경로에서도 초기화
    else:
...
if __name__ == "__main__":
    ...
    for test_name, user_request in requests_to_test.items():
        ...
        print(f"\n================ 최종 결과: {test_name} ================")
        # 최종 결과 출력 시, 반려된 경우라도 마지막 초안이 아닌 validation_result를 보여주는 것이 더 유용함
        if final_state and "accept" in final_state.get('validation_result', '').lower():
            print("🎉 최종 승인된 템플릿:")
            print(final_state['template_draft'])
        elif final_state:
            print("🔥 템플릿 생성 또는 검증 실패. 최종 결과:")
            # 실패 시에는 template_draft가 아닌 validation_result를 보여줌
            print(final_state.get('validation_result', 'N/A'))
        else:
